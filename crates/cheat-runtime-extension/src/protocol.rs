//! IPC wire protocol between the host (`cheat-runtime::extension::Extension`)
//! and the in-process extension loaded into the target via `dlopen`.
//!
//! Wire format: length-prefixed `bincode` messages. A request is `[u32 len
//! big-endian][bincode bytes]`; a response is the same shape. Both sides
//! must read the length first to know how many bytes follow. Bincode is
//! used (over JSON / postcard) because it's the default zero-config Rust
//! serialiser, has fixed-size encoding for our small enums, and the
//! producer/consumer are guaranteed-same-version (compiled together).
//!
//! All types are intentionally `Copy`-friendly where possible so the
//! extension's server loop never allocates on the steady-state path
//! (host-side allocations are fine, the target's heap is the one we want
//! to keep quiet).

use serde::{Deserialize, Serialize};

/// Default Unix socket path for an extension bound to PID `pid`. Both ends
/// must agree; the host derives it from the target PID it injected into.
pub fn socket_path_for(pid: i32) -> std::path::PathBuf {
    std::path::PathBuf::from(format!("/tmp/cheat-runtime-{pid}.sock"))
}

/// Magic + protocol version sent as a 4-byte preamble on a new connection.
...
/// guard against a misbehaving / malicious peer; legitimate cmds are well
/// under 64 KiB.
